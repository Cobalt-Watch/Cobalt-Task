...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  Widget _buildStepCountIndicator() {
    return StreamBuilder<int>(
      stream: _audioService.stepCountStream,
      initialData: _audioService.stepCount,
      builder: (context, snapshot) {
        final steps = snapshot.data ?? 0;
        if (!_audioService.isConnected) return const SizedBox.shrink();
        return Tooltip(
          message: 'Pas effectués aujourd\'hui',
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '$steps',
                  style: AppTextStyles.technicalData.copyWith(fontSize: 9),
                ),
              ],
            ),
          ),
