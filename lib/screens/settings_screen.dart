...
import '../services/tts/gemini_tts_engine.dart';
import '../services/tts/tts_engine_factory.dart';
...
class _SettingsScreenState extends State<SettingsScreen> {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // Choix du moteur de voix : local (hors-ligne) ou cloud.
          // Groq n'ayant aucun modèle TTS francophone, les options cloud sont
          // Gemini (clé déjà utilisée par le briefing navigation) et ElevenLabs.
          if (_settings.ttsEnabled) ...[
            _buildServiceRow(
              ...
              onChanged: (v) => setState(() => _settings.ttsProvider = v),
            ),

            if (_settings.ttsProvider == 'gemini')
              _buildServiceRow(
                icon: Icons.person_outline,
                color: const Color(0xFF7B4FFF),
                label: 'Timbre',
                value: _settings.ttsVoice,
                options: GeminiTtsEngine.voices,
                onChanged: (v) => setState(() => _settings.ttsVoice = v),
              ),

            if (_settings.ttsProvider != kTtsProviderLocal) _buildTtsTestRow(),
          ],
          ...
          _buildToggle(
            ...
            label: 'Vibration de confirmation',
            subtitle: 'Vibre deux fois quand une commande est exécutée',
            value: _settings.confirmationVibration,
            onChanged: (v) => setState(() => _settings.confirmationVibration = v),
          ),
          ...
          StreamBuilder<void>(
            stream: _settings.onChanged,
            builder: (context, _) {
              final configured = _settings.elevenLabsApiKey.isNotEmpty;
              return _buildAccountRow(
                icon: Icons.record_voice_over,
                ...
                subtitle: configured
                    ? 'Clé configurée'
                    : 'Optionnel — voix française plus naturelle',
                connected: configured,
                onConnect: () => showElevenLabsSetup(context),
                onDisconnect: () => showElevenLabsSetup(context),
              );
            },
          ),
  ...
  /// Ligne "Tester la voix" : synthétise une phrase sur le moteur cloud
  /// sélectionné et affiche le diagnostic.
  ///
  /// Sans ce test, une clé absente ou invalide ne se manifeste que par un
  /// repli silencieux sur la voix locale, impossible à distinguer d'un réglage
  /// resté sur "Local".
  Widget _buildTtsTestRow() {
    return Padding(
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          title: const Text('Tester la voix',
              style: TextStyle(color: AppColors.textPrimary, fontSize: 14)),
          onTap: () async {
            final messenger = ScaffoldMessenger.of(context);
            messenger.showSnackBar(
              const SnackBar(content: Text('Synthèse en cours…')),
            );
            final result = await AudioFeedbackService().testCloudEngine();
            if (!mounted) return;
            messenger
              ..hideCurrentSnackBar()
...
void showElevenLabsSetup(BuildContext context) {
  final keyController =
      TextEditingController(text: SettingsService().elevenLabsApiKey);
  final voiceController =
      TextEditingController(text: SettingsService().elevenLabsVoiceId);
      ...
      child: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Voix ElevenLabs',
                  style: TextStyle(color: Color(0xFFEEEEEE), fontSize: 18, fontWeight: FontWeight.w700)),
              const SizedBox(height: 6),
              const Text(
                'Synthèse française nettement plus naturelle que la voix locale. Le palier gratuit couvre environ 10 000 caractères par mois.',
                style: TextStyle(color: Color(0xFF888888), fontSize: 13),
              ),
              const SizedBox(height: 20),
              _navApiStep('1', 'Créez un compte ElevenLabs, puis copiez votre clé API depuis les réglages du compte.',
                  buttonLabel: 'Ouvrir ElevenLabs',
                  buttonUrl: 'https://elevenlabs.io/app/settings/api-keys'),
              _navApiStep('2', 'Laissez "Voice ID" vide pour utiliser la première voix du compte, ou collez l\'identifiant d\'une voix précise.'),
              const SizedBox(height: 4),
              _navApiKeyField(
              ...
              const SizedBox(height: 12),
              _navApiKeyField(
                label: 'Voice ID (optionnel)',
                hint: 'vide = voix par défaut du compte',
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    SettingsService().elevenLabsApiKey = keyController.text.trim();
                    SettingsService().elevenLabsVoiceId = voiceController.text.trim();
                    Navigator.pop(ctx);
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF34A853),
