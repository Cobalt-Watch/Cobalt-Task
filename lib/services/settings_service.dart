...
class SettingsService {
  ...
  // ---------------------------------------------------------------------------
  // SYSTÈME — Vibration de confirmation de commande
  // ---------------------------------------------------------------------------

  bool get confirmationVibration => _prefs?.getBool('confirmation_vibration') ?? true;
  set confirmationVibration(bool v) {
    _prefs?.setBool('confirmation_vibration', v);
    _notify();
  }
  ...
  // ---------------------------------------------------------------------------
  // TTS — Fournisseur de synthèse vocale
  // ---------------------------------------------------------------------------
  // 'local' = flutter_tts hors-ligne (défaut, gratuit), 'gemini' / 'elevenlabs'
  // = synthèse cloud. Voir lib/services/tts/tts_engine_factory.dart

  String get ttsProvider => _prefs?.getString('tts_provider') ?? 'local';
  set ttsProvider(String v) {
    _prefs?.setString('tts_provider', v);
    _notify();
  }

  /// Nom de voix Gemini ('Kore', 'Aoede'...). Ignoré par les autres moteurs.
  String get ttsVoice => _prefs?.getString('tts_voice') ?? 'Kore';
  set ttsVoice(String v) {
    _prefs?.setString('tts_voice', v);
    _notify();
  }

  // ---------------------------------------------------------------------------
  // TTS — ElevenLabs (optionnel, compte séparé)
  // ---------------------------------------------------------------------------

  String get elevenLabsApiKey => _prefs?.getString('elevenlabs_api_key') ?? '';
  set elevenLabsApiKey(String v) {
    _prefs?.setString('elevenlabs_api_key', v);
    _notify();
  }

  /// Vide = première voix du compte, résolue automatiquement via /v1/voices.
  String get elevenLabsVoiceId => _prefs?.getString('elevenlabs_voice_id') ?? '';
  set elevenLabsVoiceId(String v) {
    _prefs?.setString('elevenlabs_voice_id', v);
    _notify();
  }
}
