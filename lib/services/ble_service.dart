...
class BleService {
  ...
  /// Caractéristique Step Count (Read + Notify: nombre de pas)
  BluetoothCharacteristic? _stepCountCharacteristic;
  ...
  Future<void> _loadSelectedDeviceId() async {
    try {
      ...
      _connectedDeviceName = prefs.getString(_prefKeySelectedDeviceName);
      if (_selectedDeviceId != null) {
        // ignore: avoid_print
        print('BLE: Device ID restauré: $_selectedDeviceId (nom: $_connectedDeviceName)');
      }
    } catch (e) {
      // ignore: avoid_print
  ...
  /// Persiste le device ID et le nom sélectionnés
  Future<void> _saveSelectedDeviceId(String? deviceId, {String? deviceName}) async {
    ...
    try {
      ...
      if (deviceName != null) {
        ...
        await prefs.setString(_prefKeySelectedDeviceName, deviceName);
      } else if (deviceId == null) {
        await prefs.remove(_prefKeySelectedDeviceName);
      }
    } catch (e) {
      // ignore: avoid_print
  ...
  /// Clé SharedPreferences pour persister le nom du device sélectionné
  static const String _prefKeySelectedDeviceName = 'ble_selected_device_name';
  ...
  Future<void> connectToDevice(BluetoothDevice device, {String? deviceName}) async {
    ...
    // Mémoriser le device ID et le nom sélectionnés (persistés pour auto-reconnect)
    final resolvedName = deviceName ?? device.platformName;
    await _saveSelectedDeviceId(device.remoteId.str, deviceName: resolvedName.isNotEmpty ? resolvedName : null);
    _connectedDeviceName = resolvedName.isNotEmpty ? resolvedName : _connectedDeviceName;
    ...
    for (final service in services) {
      ...
      if (serviceUuid == BleConstants.serviceUuid.toLowerCase()) {
        ...
        for (final characteristic in service.characteristics) {
          ...
          // Caractéristique Step Count (Read + Notify)
          if (charUuid == BleConstants.stepCountCharacteristicUuid.toLowerCase()) {
            _stepCountCharacteristic = characteristic;
            // ignore: avoid_print
            print('BLE:   -> Step Count trouvée!');
          }
        }
      }
  ...
  Future<void> _subscribeToNotifications() async {
    ...
    // Souscrire aux notifications Step Count et lire la valeur initiale
    if (_stepCountCharacteristic != null) {
      try {
        final initial = await _stepCountCharacteristic!.read();
        if (initial.length >= 4) {
          _stepCount = ByteData.sublistView(Uint8List.fromList(initial))
              .getUint32(0, Endian.little);
          _stepCountController.add(_stepCount);
          // ignore: avoid_print
          print('BLE: Pas initial = $_stepCount');
        }
      } catch (e) {
        // ignore: avoid_print
        print('BLE: Erreur lecture step count initiale: $e');
      }
      await _stepCountCharacteristic!.setNotifyValue(true);
      _stepCountSubscription?.cancel();
      _stepCountSubscription =
          _stepCountCharacteristic!.onValueReceived.listen((data) {
        if (data.length >= 4) {
          _stepCount = ByteData.sublistView(Uint8List.fromList(data))
              .getUint32(0, Endian.little);
          _stepCountController.add(_stepCount);
          // ignore: avoid_print
          print('BLE: Step count = $_stepCount');
        }
      });
      // ignore: avoid_print
      print('BLE: Abonné aux notifications Step Count');
    }
  }
